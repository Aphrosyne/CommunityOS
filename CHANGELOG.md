...
- **Database Round 4 — 审核日志结构化记录**：新增 `DatabaseManager.log_moderation()` Repository 函数（含模块级委托），`mute.py`（禁言/解禁/自禁/拒绝禁言）、`auto_recall.py`（违禁词自动撤回成功与失败）、`admin.py`（权限设置成功与拒绝）接入写入 `moderation_log` 表。`details` 字段 JSON 序列化（`json.dumps(ensure_ascii=False, default=str)`），`details=None` 写入 NULL；运行时 DB 写入失败仅记日志不抛出。新增 `tests/unit/test_moderation_log.py`（11 个用例）。
- **Database Round 5 — 指令日志结构化记录**：新增 `DatabaseManager.log_command()` Repository 函数（含模块级委托），`command_dispatcher.py` 在指令执行成功（result=success）和异常（result=error）时写入 `command_log` 表。`MatcherException`（NoneBot 控制流如 finish/pause/reject）视为正常完成记录 success 并 re-raise。`raw_text` Python 端截断至 200 字符，私聊 `group_id=0`，shortcut 命中按最终命令名记录。黑名单/冷却/权限拒绝/未注册命令不写入。新增 `tests/unit/test_command_log.py`（12 个用例）。
- **群级管理员指令 `/groupadmin`**（别名 `sga`）：BotAdmin 可在群内分配群级管理员（level=2，仅当前群生效），区别于 `/botadmin`（全局 level=3，Owner 专用）。`_apply` 新增 `group_id` 参数支持群级作用域，回复带作用域标识（"群XXX"/"全局"）。
- **权限查询指令 `/perm`**（别名 `spm`）：支持查看用户所有权限记录（`/perm @user`）、清除所有权限记录（`/perm @user clear`，Owner 保护）、查询指定群有效权限（`/perm @user in <群号>`）。新增 `get_user_permissions()` 和 `clear_user_permissions()` Repository 函数。
- **合并转发消息自动撤回**：`auto_recall.py` 增加 `forward` 消息段类型检测，当群关键词列表含 `[合并转发消息]` 标记且消息含 forward 段时触发撤回（QQ 客户端显示的"[聊天记录]"/"[转发消息]"是本地渲染，协议层为 forward 段，`get_plaintext()` 返回空）。

### Changed

- **插件自动扫描加载**：`main.py` 将手动 `load_plugin` 列表改为自动扫描 `plugins/*.py`（跳过 `_` 前缀文件），保持 `load_plugin("plugins.{stem}")` 模块名解析避免 ImportError。
- **权限指令别名改为英文缩写**：`botadmin`→`sba`、`groupadmin`→`sga`、`whitelist`→`swl`、`blacklist`→`sbl`、`perm`→`spm`（替换原中文别名）。
- **MANAGED_GROUPS 从 .env 迁移到 runtime.json**：配置格式从逗号分隔字符串改为 JSON 数组，支持 `/reload` 热更新无需重启。`auto_recall.py`/`member.py`/`publish.py`/`message_rule.py` 改为 `runtime_config.get("MANAGED_GROUPS", [])`。
- `docs/design/database.md`：§3.2 更新 `moderation_log`（action 值域、无 FK、JSON details）和 `command_log`（私聊 group_id=0、raw_text 截断 200、无 FK）字段说明。

### Removed

- `moderation_log` 表 `user_id`、`operator_id` 的外键约束（审计日志优先级高于数据完整性约束，保证被操作用户不存在或系统操作 operator_id=0 时仍能写入）。
- `command_log` 表 `user_id` 的外键约束（指令日志是行为审计，未注册用户也可执行指令）。
- `greet` 插件（重命名为 `greet.py.disabled`，自动扫描不再加载）。
- `.env` 和 `.env.example` 的 `MANAGED_GROUPS` 配置项（迁移至 runtime.json）。
- `services/config.py` 的 `MANAGED_GROUPS` 读取代码。
